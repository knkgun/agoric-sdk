...
export function makeVirtualReferenceManager(
   ...
   */
  const reanimatorTable = new Map();

  /**
   * Register a reanimator function for a virtual object kind.
   *
   * @param {string} kindID  The kind of virttual object the reanimator will reanimate
   * @param {(string, boolean) => Object} reanimator  Reanimator function for the given kind.
   */
  function registerReanimator(kindID, reanimator) {
    reanimatorTable.set(kindID, reanimator);
  }

  /**
   * Create an in-memory representative given a virtual object ID by reanimating
   * it from persistent storage.  Used for deserializing.
   *
  ...
  function reanimate(vobjID, proForma) {
    ...
    const kindID = `${id}`;
    const reanimator = reanimatorTable.get(kindID);
    if (reanimator) {
